...
class HistoryProvider extends ChangeNotifier {
  ...
  Future<String?> fetchHistoricalData(
    CurrencyModel? base,
    ...
    chartSpots.clear();
    try {
      historicalData = await _apiService.getHistoricalRates(
        base.code,
        target.code,
        days: days,
      );
      ...
      if (_cache.containsKey(cacheKey)) {
        ...
        errorMessage =
            'Failed to load historical data. Please check your connection.';
        return null;
      }
    } finally {
      _setLoading(false);
  ...
  void _prepareChartData() {
    ...
    for (int i = 0; i < sortedKeys.length; i++) {
      ...
      if (i > 0) {
        ...
        final int steps = selectedDays <= 10
            ? 20
            : (selectedDays <= 35 ? 10 : 2);
